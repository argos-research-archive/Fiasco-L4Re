...
void Lua::Lib::run_init(lua_State *l)
{
...
int lua(int argc, char const * const *argv)
{
	...
	int opt;
	while ((opt = getopt_long(argc, const_cast<char * const *>(argv), options,
			loptions, NULL)) != -1)
	{
		switch (opt)
		{
	...
	if (luaL_loadbuffer(L, _binary_ned_lua_start,
			_binary_ned_lua_end - _binary_ned_lua_start, "@ned.lua"))
	{
		printf("Ned: script: ---\n%.*s\n---",
				(int) (_binary_ned_lua_end - _binary_ned_lua_start),
